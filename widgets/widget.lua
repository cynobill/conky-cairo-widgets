...
Widget = Class(function(a, name, args)
	...
	if type(args.mask_image) == 'string' then
		a.mask_image = args.mask_image
	else
		a.mask_image = false
	end
...
function Widget:toString()
	...
	print(" background image: "..tostring(self.background))
	print(" mask image: "..tostring(self.mask_image))
...
function Widget:_get_source(context)
	if self.pattern == nil then
		cairo_save(context)
		print(cairo_user_to_device(context, 1, 1)
		cairo_identity_matrix(context)
		print(cairo_user_to_device(context, 1, 1)
		cairo_scale(context,1/self.w, 1/self.h)
		...
		if self.background == false then
			...
			local surface = cairo_image_surface_create_from_png('images/'..self.background)
...
function Widget:_get_mask(context)
	if self.mask == nil then
		...
		if self.mask_image == false then 
			...
			local surface = cairo_image_surface_create_from_png('images/'..self.mask_image)
		...
		cairo_restore(context) --restore our scaling
	end
	return self.mask
end
...
function Widget:_draw_border(context)
	...
	if self.border_width > 0 then
		...
		cairo_rectangle( context, 0, 0, 1.0, 1.0)
...
--- Placeholder function, overrride in child class.
function Widget:_update()
	return
end


--- Main drawing code for the widget
-- @context The cairo context to use.
function Widget:_draw_widget(context)
	--- Just paint the widget with the source using the mask
